$(function () {
  ...
  // This is @instagrams account ID, the instragrammer that
  // we're following for this coding assignment.
  var USER_ID = '17841403377586680';

  window.Instagram.getUser(USER_ID, (userData) => {
  ...
  window.Instagram.getUserRecentMedia(USER_ID, (media) => {
    ...
    const filteredTags = filterTagsByFrequency(sortedTags, 2)
  ...
  function sortTagsByFrequencyDesc(tags) {
    // sorting of tags by frequency
    //const sortedArray = Array.from(tags).sort(()=>{})
    //const sortedArray = tags.slice().sort(()=>{})
    const sortedArray = [...tags].sort((a, b)=>b.frequency - a.frequency);
  ...
  function filterTagsByFrequency(tags, minFrequency) {
    return tags.filter((tag)=>tag.frequency >= minFrequency)
  }
  ...
  function renderTags(tags) {
    ...
    var $a = $('<a>').html(`#allTags (${userMedia.length})`);
    const $li = $('<li>').addClass('u-pull-left').append($a);
    tagList.append($li);
    $a.data('tagId', '');
    ...
    tags.forEach(function (tag) {
      ...
      // eg. '#canon (15)'
      var $a = $('<a>').html(`#${tag.id} (${tag.frequency})`);
  ...
  function handleClickTag(event) {
    ...
    const filteredMedia = filterMediaByTag(tagId)
  ...
  function filterMediaByTag (tagId) {
    if (tagId === '') {
      ...
      return userMedia.filter((mediaItem) => mediaItem.tags.includes(tagId));
    }
  }
});
...
/* 
Improvement Ideas:
- Visual highlight of applied Tag filter (disappears once other is clicked or filter is deactivated)
- Second click on tag => deactivate filter
- Capability to apply a filter including multiple tags 'e.g. show pictures with tags 'barcelona' and/or 'coding'
- Hover over tiles => applied tags highlighted
- Make the image a bit bigger on hover
*/
